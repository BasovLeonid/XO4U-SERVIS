        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Редактирование рекламного аккаунта</h3>
                ...
                <div class="card-body">
                    ...
                    <form id="editAccountForm" action="{{ route('boss.accounts.update', $account) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" 
                                ...
                                <option value="yandex" {{ old('type', $account->type) == 'yandex' ? 'selected' : '' }}>Яндекс Директ</option>
                                <option value="vk" {{ old('type', $account->type) == 'vk' ? 'selected' : '' }}>ВК Реклама</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="subtype" 
                                ...
                                <option value="created" {{ old('subtype', $account->subtype) == 'created' ? 'selected' : '' }}>Созданный нами</option>
                                <option value="added" {{ old('subtype', $account->subtype) == 'added' ? 'selected' : '' }}>Добавленный пользователем</option>
                            </select>
                            <small class="form-text text-muted">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="oauth_token" 
                                      ...
                                      rows="3"
                                      placeholder="Введите OAuth токен, если он есть">{{ old('oauth_token', $account->oauth_token) }}</textarea>
                            <small class="form-text text-muted">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" 
                                    ...
                                    required>
                                <option value="active" {{ old('status', $account->status) == 'active' ? 'selected' : '' }}>Активный</option>
                                <option value="archived" {{ old('status', $account->status) == 'archived' ? 'selected' : '' }}>Архивный</option>
                                <option value="paused" {{ old('status', $account->status) == 'paused' ? 'selected' : '' }}>На паузе</option>
                            </select>
                            <small class="form-text text-muted">
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="user_id" id="user_id" value="{{ old('user_id', $account->user_id) }}" required>
                            @error('user_id')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
