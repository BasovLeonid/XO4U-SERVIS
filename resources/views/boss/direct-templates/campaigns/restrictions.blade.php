...
@section('content')
<x-boss.direct-templates.container :title="'Ограничения кампании \'' . $campaign->name . '\''">
    <x-slot:sidebar>
        <x-yandex-direct.campaigns.sidebar :campaign="$campaign" />
    </x-slot:sidebar>

    <form action="{{ route('boss.direct-templates.campaigns.update', [$template, $campaign]) }}" method="POST" id="restrictionsForm">
        @csrf
        @method('PUT')

        <x-yandex_direct.campaigns.restrictions />
